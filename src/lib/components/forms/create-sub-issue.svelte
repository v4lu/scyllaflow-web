<script lang="ts">
	...
	import type { CreateTag, Project, SubIssueRequest } from '$lib/types/issue.type';
	...
	type Props = {
		...
		authToken: string;
		workspace: Workspace;
		createIssue: (payload: SubIssueRequest) => Promise<void>;
	...
	const { createTag } = useWorkspaceIssues(authToken, workspace.slug);
